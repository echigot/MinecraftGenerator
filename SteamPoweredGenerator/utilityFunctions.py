...
def matrix(level, box):
    x, y, z = getBoxSize(box)
    posx=box.minx
    posy=box.miny
    posz=box.minz
    matrix = [[[(0,0) for k in xrange(z)] for j in xrange(y)] for i in xrange(x)]
    updated = [[[False for k in xrange(z)] for j in xrange(y)] for i in xrange(x)]
    
    for i in range(posx,posx+x):
        for j in range(posy,posy+y):
            for k in range(posz, posz+z):
                matrix[i-posx][j-posy][k-posz]=(level.blockAt(i,j,k), level.blockDataAt(i,j,k))
    return matrix, updated, x,y,z


def updateWorld(level,box, matrix, updated):
    x, y, z = getBoxSize(box)
    posx=box.minx
    posy=box.miny
    posz=box.minz
